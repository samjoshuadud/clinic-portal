...
function PatientProfile() {
  ...
  const fetchPatient = async (patientId) => {
    try {
      const response = await axios.get(`https://clinic-portal.onrender.com//patients/${patientId}`);
      const patientData = {
        id: response.data.id,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`https://clinic-portal.onrender.com//patients/${patient.id}`, {
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`https://clinic-portal.onrender.com//patients/${patient.id}`);
      if (response.status === 200) {
